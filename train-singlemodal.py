...
def parse_args():
    parser = argparse.ArgumentParser(description='YOLO11 Single Modal Training Script')

    # Model and dataset config
    parser.add_argument('--mode', type=str, default='singlemodal-rgb', help='Training mode')
    ...
    parser.add_argument('--close_mosaic', type=int, default=30, help='Disable mosaic augmentation after N epochs')
    ...
    # parser.add_argument('--use_simotm', type=str, default='RGB', help='Modality type')
...
if __name__ == '__main__':
    ...
    model_yaml = f'./ultralytics/cfg/models/11/yolo11.yaml'
    model = YOLO(model_yaml)
    print(f'Using model config: {model_yaml}')

    # Select data config based on dataset
    if args.dataset == 'M3FD' and args.mode == 'singlemodal-rgb':
        data_yaml = './M3FD-rgbt.yaml'
        use_simotm = 'RGB'
    elif args.dataset == 'M3FD' and args.mode == 'singlemodal-t':
        data_yaml = './M3FD-singlemodal.yaml'
        use_simotm = 'SimOTM'
    print(f'Using data yaml: {data_yaml}', f'use_simotm: {use_simotm}')
